...
my $sth = $dbh->prepare("UPDATE radpawsusers SET firstname=?, lastname=?, housenumber=?, streetname=?, postcode=?, telephone=?, email=?, passwordquestion=?, passwordanswer=?, signup=?, notes=? WHERE username=?") or &dbdie; 
$sth->execute($firstname, $lastname, $housenumber, $streetname, $postcode, $telephone, $email,  $passwordquestion, $passwordanswer, $signup, $notes, $username) or &dbdie;
...
my $rec;
my $user_active = "yes";
my $sth = $dbh->prepare("select * from radcheck where username = ? and attribute = 'Auth-Type' and value = 'Reject'") or &dbdie;
$sth->execute($username) or &dbdie;
...
my $send_email = 0;
my $email_message = "";
if ($activation eq "yes" && $user_active eq "no") {
	$sth = $dbh->prepare("DELETE FROM radcheck  WHERE username=? AND attribute = 'Auth-Type'") or &dbdie; 
	$sth->execute($username) or &dbdie;
	$send_email = 1;
	$email_message = "activated and is ready to be used. Please visit https://vpn.publicaccesswifi.org/secure-cgi/vpn/instructions.cgi for instructions on how to configure your device.";
}
if ($activation eq "" && $user_active eq "yes") {
	$sth = $dbh->prepare("INSERT INTO radcheck  VALUES (NULL, ?, 'Auth-Type', ':=', 'Reject')") or &dbdie;
	$sth->execute($username) or &dbdie;
	$send_email = 1;
	$email_message = "de-activated and cannot be used at present. Please contact the PAWS team for further details.";
...
if ($send_email) {
	my $sender = new Mail::Sender {
        ...
        $sender->Open({
                to => "$email",
                cc => 'C.Greiffenhagen@lboro.ac.uk, Murray.Goulden@nottingham.ac.uk, mvr@cs.nott.ac.uk',
                subject => 'User Self-Sign-Up'
        })
                or die "Can't open the message: $sender->{'error_msg'}\n";
        $sender->SendLineEnc("Dear $firstname $lastname,\n\n");
        $sender->SendLineEnc("Your account ($username) has been $email_message \n\n");
        $sender->SendLineEnc("\n\nBest regards,\nThe PAWS Team\n");
        $sender->Close()
...
if (defined $un) {
	...
	print "<h1>Update for user $rec->{username}:</h1>";
	print start_form(
		-action => '/secure-cgi/adm/eu.cgi'
		);
	...
	Tr(
		td('<b>Account status</b>'),
		td($ua)
	),
        Tr(
                td('<b>Account created on</b>'),
                td("<b>$rec->{ts}</b>")
        ),


	Tr(
		td("$ua_tick to $ua_action") ,
		td(
			checkbox(
				-name    => 'activation',
				-checked => $ua_status,
